...
class StickerBSFragment(val frgcontext: Context) : BottomSheetDialogFragment() {
    ...
    fun load_image() {
        ...
        val query =frgcontext.contentResolver.query(
    ...
    override fun setupDialog(dialog: Dialog, style: Int) {
        ...
        rvEmoji.setItemViewCacheSize(list_image.size)
    }

    inner class StickerAdapter : RecyclerView.Adapter<StickerAdapter.ViewHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            val view = LayoutInflater.from(parent.context).inflate(R.layout.row_img, parent, false)
            return ViewHolder(view)
        }

        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            val contentUri = MediaStore.Images.Media.EXTERNAL_CONTENT_URI
            val uri = contentUri.buildUpon().appendPath(list_image[position].id.toString()).build()
            ...
            init {
                itemView.setOnClickListener {
                    if (mStickerListener != null) {
                        val contentUri = MediaStore.Images.Media.EXTERNAL_CONTENT_URI
                        val uri = contentUri.buildUpon().appendPath(list_image[layoutPosition].id.toString()).build()
                        Glide.with(requireContext())
                                .asBitmap()
                                .load(uri)
