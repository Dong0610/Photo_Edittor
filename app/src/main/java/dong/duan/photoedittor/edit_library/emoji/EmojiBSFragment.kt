...
class EmojiBSFragment(val fragmentContext: Context) : BottomSheetDialogFragment() {
    ...
    override fun setupDialog(dialog: Dialog, style: Int) {
        ...
        rvEmoji.setItemViewCacheSize(getEmojis(fragmentContext).size)
    }
    ...
    inner class EmojiAdapter : RecyclerView.Adapter<EmojiAdapter.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            holder.txtEmoji.text = getEmojis(fragmentContext)[position]
        }

        override fun getItemCount(): Int {
            return getEmojis(fragmentContext).size
        }
            ...
            init {
                itemView.setOnClickListener {
                    if (mEmojiListener != null) {
                        mEmojiListener!!.onEmojiClick(getEmojis(fragmentContext)[layoutPosition])
                    }
                    dismiss()
                }
            }
